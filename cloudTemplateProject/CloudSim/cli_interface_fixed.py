...
class DistributedStorageCLI(cmd.Cmd):
    ...
    def do_start(self, arg):
        ...
        # Create a shared network manager for all nodes
        from network_manager import NetworkManager
        self.network_manager = NetworkManager(use_localhost=True)
        ...
        for node_id, port in node_configs:
            try:
                # Create node with shared network manager
                node = VirtualNode(node_id, capacity_gb=2, port=port, use_localhost=True, shared_network_manager=self.network_manager)
        ...
        if len(self.nodes) == 5:
            ...
            self.network_manager.start_heartbeat_monitor()
    ...
    def do_discover(self, arg):
        ...
        if discovered:
            ...
            for node_id in discovered:
                status = "ACTIVE" if self.nodes[node_id].is_active else "INACTIVE"
    ...
    def do_shutdown(self, arg):
        ...
        for node_id, node in self.nodes.items():
            try:
                node.shutdown()
                print(f"   SUCCESS: {node_id} shutdown")
            except Exception as e:
